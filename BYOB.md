...
The [slsa-github-generator](https://github.com/Kong/slsa-github-generator) repository hosts the code for the BYOB framework maintained by the OpenSSF SLSA tooling team. There are two main components you will use for your integration, the SLSA Setup Action and the SLSA Reuseable Workflow.

#### SLSA Setup Action (SSA)

The [setup-generic](https://github.com/Kong/slsa-github-generator/blob/main/actions/delegator/setup-generic) Action is used to initialize the BYOB framework. It returns a so-called "SLSA token" which is used in later steps:

```yaml
- uses: Kong/slsa-github-generator/actions/delegator/setup-generic@v2_node20
```
...
```yaml
- uses: Kong/slsa-github-generator/.github/workflow/delegator_generic_slsa3.yml@v2_node20
  with:
...
slsa-run:
  ...
  uses: Kong/slsa-github-generator/.github/workflows/delegator_generic_slsa3.yml@v2_node20
  with:
    slsa-token: ${{ needs.slsa-setup.outputs.slsa-token }}
  secrets:
...
- Pin your depenencies by hash except the [delegator workflow](https://github.com/Kong/slsa-github-generator/tree/main#referencing-slsa-builders-and-generators), to avoid dependency confusion attacks and speed up incidence response.
