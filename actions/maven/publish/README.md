...
Before using the Maven publish action, you should have a workflow that invokes the [Maven builder](https://github.com/Kong/slsa-github-generator/blob/main/internal/builders/maven/README.md). It will look something like this:
...
jobs:
  build:
    permissions:
      ...
      contents: read
      actions: read
    uses: Kong/slsa-github-generator/.github/workflows/builder_maven_slsa3.yml@v2_node20
    with:
...
publish:
  ...
  steps:
    - name: publish
      id: publish
      uses: Kong/slsa-github-generator/actions/maven/publish@v2_node20
      with:
