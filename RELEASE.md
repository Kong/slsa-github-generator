...
# This is the existing slsa-verifier version used by the builder. (https://github.com/Kong/slsa-github-generator/blob/main/.github/actions/generate-builder/action.yml#L55)
...
This will trigger the [release workflow](https://github.com/Kong/slsa-github-generator/actions/workflows/release.yml). Cancel this in the [UI](https://github.com/Kong/slsa-github-generator/actions/workflows/release.yml).
...
This will trigger the [release workflow](https://github.com/Kong/slsa-github-generator/actions/workflows/release.yml). Ensure this workflow succeeds and that the release assets are updated.
...
1. Make sure you have downloaded the `$BUILDER_TAG` builder's binary locally `slsa-builder-go-linux-amd64`, either via the web UI or via:
   ...
   ```shell
   "$GH" release -R Kong/slsa-github-generator download "$BUILDER_TAG" -p "slsa-builder-go-linux-amd64"
...
2. Upload a different binary to the assets:
   ...
   "$GH" release -R Kong/slsa-github-generator upload "$BUILDER_TAG" slsa-builder-go-linux-amd64  --clobber
   ```
...
6. Re-run the workflow above and verify that it succeeds. (TODO: https://github.com/Kong/slsa-github-generator/issues/116).
...
1. Make sure you have downloaded the `$BUILDER_TAG` builder's binary locally `slsa-generator-generic-linux-amd64`, either via the web UI or via:
   ...
   ```shell
   "$GH" release -R Kong/slsa-github-generator download "$BUILDER_TAG" -p "slsa-generator-generic-linux-amd64"
...
2. Upload a different binary to the assets:
   ...
   "$GH" release -R Kong/slsa-github-generator upload "$BUILDER_TAG" slsa-generator-generic-linux-amd64  --clobber
   ```
...
1. Make sure you have downloaded the `$BUILDER_TAG` builder's binary locally `slsa-generator-container-linux-amd64`, either via the web UI or via:
   ...
   ```shell
   "$GH" release -R Kong/slsa-github-generator download "$BUILDER_TAG" -p "slsa-generator-container-linux-amd64"
...
2. Upload a different binary to the assets:
   ...
   "$GH" release -R Kong/slsa-github-generator upload "$BUILDER_TAG" slsa-generator-container-linux-amd64  --clobber
   ```
...
1. Make sure you have downloaded the `$BUILDER_TAG` builder's binary locally `slsa-builder-docker-linux-amd64`, either via the web UI or via:
   ...
   ```shell
   "$GH" release -R Kong/slsa-github-generator download "$BUILDER_TAG" -p "slsa-builder-docker-linux-amd64"
...
2. Upload a different binary to the assets:
   ...
   "$GH" release -R Kong/slsa-github-generator upload "$BUILDER_TAG" slsa-builder-docker-linux-amd64  --clobber
   ```
...
Create a new tag for the final release via [Kong/slsa-github-generator/releases/new](https://github.com/Kong/slsa-github-generator/releases/new). The tag _MUST_ be a "canonical" [semantic version](https://semver.org/) without metadata (`$BUILDER_TAG`). Shorter versions are not accepted by the builder's
and verifier's code.
...
<!-- TODO(github.com/Kong/slsa-github-generator/issues/1110): Describe GHA generic container e2e tests. -->
...
1. Generate binaries and provenance in
   ...
   ```yaml
   uses: Kong/slsa-github-generator/.github/workflows/builder_go_slsa3.yml@<BUILDER_TAG>
   ```
